...
def inspect_csv_state() -> str:
    ...
    if header != CSV_HEADERS:
        print(f"WARNING: Unexpected CSV header: {header}. Rebuilding full history.")
        return "empty"
...
def fetch_batch(end_timestamp: str | None = None) -> tuple[dict, list[dict]]:
    params = {
        "api_key": require_env("TS_API_KEY", TS_API_KEY),
        "results": BATCH_SIZE,
    }
    if end_timestamp:
        params["end"] = end_timestamp.removesuffix("Z")
...
def sync_full_history() -> None:
    print(f"DEBUG: Starting full sync into {CSV_FILE}")
